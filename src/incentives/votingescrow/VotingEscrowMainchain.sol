...
contract VotingEscrowMainchain is VotingEscrowTokenBase, IVotingEscrowMainchain, MsgSenderAppUpd {
    ...
    error ExpiryInThePast(uint128 expiry);
    ...
    function increaseLockPosition(
        ...
        if (!WeekMath.isValidWTime(newExpiry)) revert InvalidWTime(newExpiry);
        if (MiniHelpers.isTimeInThePast(newExpiry)) revert ExpiryInThePast(newExpiry);

        if (newExpiry < positionData[user].expiry) revert VENotAllowedReduceExpiry();
    ...
    function _increasePosition(
        ...
        LockedPosition memory newPosition = LockedPosition(
            oldPosition.amount + amountToIncrease, oldPosition.expiry + durationToIncrease
        );
