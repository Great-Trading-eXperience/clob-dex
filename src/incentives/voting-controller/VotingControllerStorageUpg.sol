...
abstract contract VotingControllerStorageUpg is IVotingController {
    ...
    error InvalidWTime(uint128 wTime);
    ...
    uint128 public constant GOVERNANCE_VOTE = 10 * (10 ** 6) * (10 ** 18); // 10 mils of PENDLE

    IVeToken public immutable veToken;

    uint128 public deployedWTime;

    uint128 public tokenPerSec;
        ...
        )
    {
        ...
        (chainId, lastSlopeChangeAppliedAt, totalVote) =
            (data.chainId, data.lastSlopeChangeAppliedAt, data.totalVote);

        slopeChanges = new uint128[](wTimes.length);
        for (uint256 i = 0; i < wTimes.length; ++i) {
            if (!wTimes[i].isValidWTime()) revert InvalidWTime(wTimes[i]);
    ...
    function getWeekData(
        ...
        if (!wTime.isValidWTime()) revert InvalidWTime(wTime);
    ...
    function getAllRemovedPools(
        ...
        if (end >= lengthOfRemovedPools) revert ArrayOutOfBounds();
