...
contract BalanceAndFeeTest is Test {
    ...
    function testMultipleMakersSingleTakerFullOrderMatching() public {
        ...
        uint256 feeCollectorInitialBaseBalance = balanceManager.getBalance(
            feeCollector,
            baseCurrency
        );
        uint256 feeCollectorInitialQuoteBalance = balanceManager.getBalance(
            feeCollector,
            quoteCurrency
        );

        // Define order parameters
        Price alicePrice = Price.wrap(2505 * (10 ** quoteDecimals)); // Best price
        Price charliePrice = Price.wrap(2502 * (10 ** quoteDecimals)); // Middle price
        Price davidPrice = Price.wrap(2500 * (10 ** quoteDecimals)); // Lowest price

        Quantity aliceQuantity = Quantity.wrap(
            uint128(2505 * (10 ** baseDecimals)) / 2505
        ); // 3000 USDC
        Quantity bobQuantity = Quantity.wrap(uint128(3 * (10 ** baseDecimals))); // 4500 USDC
        Quantity charlieQuantity = Quantity.wrap(
            uint128(2502 * (10 ** baseDecimals)) / 2502
        ); // 4500 USDC
        Quantity davidQuantity = Quantity.wrap(
            uint128(2500 * (10 ** baseDecimals)) / 2500
        ); // 5000 USDC
        ...
        uint256 aliceLockedBeforeOrder = balanceManager.getLockedBalance(
            alice,
            address(orderBook),
            quoteCurrency
        );
        ...
        vm.startPrank(alice);
        console.log(
            "--- Alice places buy order at 2505 USDC per WETH (2505 USDC) ---"
        );
        ...
        uint256 aliceLockedAfterOrder = balanceManager.getLockedBalance(
            alice,
            address(orderBook),
            quoteCurrency
        );
        assertEq(
            aliceLockedAfterOrder,
            aliceExpectedLocked,
            "Alice's locked amount incorrect"
        );
        ...
        console.log(
            "--- Charlie places buy order at 2502 USDC per WETH (2502 USDC) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        uint256 charlieLockedAfterOrder = balanceManager.getLockedBalance(
            charlie,
            address(orderBook),
            quoteCurrency
        );
        assertEq(
            charlieLockedAfterOrder,
            charlieExpectedLocked,
            "Charlie's locked amount incorrect"
        );

        vm.startPrank(david);
        console.log(
            "--- David places buy order at 2500 USDC per WETH (2500 USDC) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        uint256 davidLockedAfterOrder = balanceManager.getLockedBalance(
            david,
            address(orderBook),
            quoteCurrency
        );
        assertEq(
            davidLockedAfterOrder,
            davidExpectedLocked,
            "David's locked amount incorrect"
        );
        ...
        uint256 expectedMatchVolumeInBase = Quantity.unwrap(aliceQuantity) +
            Quantity.unwrap(charlieQuantity) +
            Quantity.unwrap(davidQuantity);
        uint256 expectedMatchVolumeInQuote = ((Quantity.unwrap(aliceQuantity) *
            Price.unwrap(alicePrice)) +
            (Quantity.unwrap(charlieQuantity) * Price.unwrap(charliePrice)) +
            (Quantity.unwrap(davidQuantity) * Price.unwrap(davidPrice))) /
            (10 ** baseDecimals);

        // Breaking down calculations to reduce stack depth
        uint256 aliceQuantityUnwrapped = Quantity.unwrap(aliceQuantity);
        uint256 alicePriceUnwrapped = Price.unwrap(alicePrice);
        uint256 aliceTradeValue = (alicePriceUnwrapped *
            aliceQuantityUnwrapped) / (10 ** baseDecimals);

        uint256 charlieQuantityUnwrapped = Quantity.unwrap(charlieQuantity);
        uint256 charliePriceUnwrapped = Price.unwrap(charliePrice);
        uint256 charlieTradeValue = (charliePriceUnwrapped *
            charlieQuantityUnwrapped) / (10 ** baseDecimals);

        uint256 davidQuantityUnwrapped = Quantity.unwrap(davidQuantity);
        uint256 davidPriceUnwrapped = Price.unwrap(davidPrice);
        uint256 davidTradeValue = (davidPriceUnwrapped *
            davidQuantityUnwrapped) / (10 ** baseDecimals);

        uint256 totalTradeValue = aliceTradeValue +
            charlieTradeValue +
            davidTradeValue;
        ...
        uint256 aliceMakerFee = (Quantity.unwrap(aliceQuantity) * feeMaker) /
            feeUnit;
        uint256 charlieMakerFee = (Quantity.unwrap(charlieQuantity) *
            feeMaker) / feeUnit;
        uint256 davidMakerFee = (Quantity.unwrap(davidQuantity) * feeMaker) /
            feeUnit;
        uint256 totalMakerFee = aliceMakerFee + charlieMakerFee + davidMakerFee;

        uint256 bobQuantityUnwrapped = Quantity.unwrap(bobQuantity);
        uint256 bobMakerFee = (bobQuantityUnwrapped * feeMaker) / feeUnit;

        // Verify order book has correct orders
        (uint48 orderCountAlice, uint256 volumeAlice) = orderBook.getOrderQueue(
            Side.BUY,
            alicePrice
        );
        (uint48 orderCountCharlie, uint256 volumeCharlie) = orderBook
            .getOrderQueue(Side.BUY, charliePrice);
        (uint48 orderCountDavid, uint256 volumeDavid) = orderBook.getOrderQueue(
            Side.BUY,
            davidPrice
        );
        ...
        assertEq(
            volumeAlice,
            aliceQuantityUnwrapped,
            "Volume at Alice's price incorrect"
        );
        assertEq(
            volumeCharlie,
            charlieQuantityUnwrapped,
            "Volume at Charlie's price incorrect"
        );
        assertEq(
            volumeDavid,
            davidQuantityUnwrapped,
            "Volume at David's price incorrect"
        );
        ...
        uint256 feeCollectorBaseBalance = balanceManager.getBalance(
            feeCollector,
            baseCurrency
        );
        uint256 feeCollectorQuoteBalance = balanceManager.getBalance(
            feeCollector,
            quoteCurrency
        );

        uint256 baseBalanceDiff = feeCollectorBaseBalance -
            feeCollectorInitialBaseBalance;
        uint256 quoteBalanceDiff = feeCollectorQuoteBalance -
            feeCollectorInitialQuoteBalance;

        assertApproxEqAbs(
            baseBalanceDiff,
            bobMakerFee,
            100,
            "Fee collector base balance incorrect"
        );
        assertApproxEqAbs(
            quoteBalanceDiff,
            bobTakerFee,
            100,
            "Fee collector quote balance incorrect"
        );
        ...
        uint256 bobCurrentBaseBalance = baseToken.balanceOf(bob) +
            balanceManager.getBalance(bob, baseCurrency);
        uint256 bobCurrentQuoteBalance = quoteToken.balanceOf(bob) +
            balanceManager.getBalance(bob, quoteCurrency);

        uint256 bobBaseDecrease = bobInitialBaseBalance - bobCurrentBaseBalance;
        uint256 bobQuoteIncrease = bobCurrentQuoteBalance -
            bobInitialQuoteBalance;

        assertApproxEqAbs(
            bobBaseDecrease,
            expectedMatchVolumeInBase,
            100,
        ...
        assertApproxEqAbs(
            bobQuoteIncrease,
            expectedMatchVolumeInQuote - bobTakerFee,
            100,
            "Bob's quote token increase incorrect"
        );

        // 3. Verify Alice received her base tokens
        uint256 aliceCurrentBaseBalance = baseToken.balanceOf(alice) +
            balanceManager.getBalance(alice, baseCurrency);
        uint256 aliceCurrentQuoteBalance = quoteToken.balanceOf(alice) +
            balanceManager.getBalance(alice, quoteCurrency);

        uint256 aliceBaseIncrease = aliceCurrentBaseBalance -
            aliceInitialBaseBalance;
        uint256 aliceQuoteDecrease = aliceInitialQuoteBalance -
            aliceCurrentQuoteBalance;

        assertApproxEqAbs(
            aliceBaseIncrease,
            aliceQuantityUnwrapped - aliceMakerFee,
            100,
        ...
        assertApproxEqAbs(
            ...
            aliceExpectedLocked,
            100,
            "Alice's quote token decrease incorrect"
        );

        // 4. Verify Alice's locked quote tokens were spent
        uint256 aliceRemainingLocked = balanceManager.getLockedBalance(
            alice,
            address(orderBook),
            quoteCurrency
        );
        assertEq(
            aliceRemainingLocked,
            0,
            "Alice should have no remaining locked balance"
        );

        // 5. Verify Charlie received his base tokens
        uint256 charlieCurrentBaseBalance = baseToken.balanceOf(charlie) +
            balanceManager.getBalance(charlie, baseCurrency);
        uint256 charlieCurrentQuoteBalance = quoteToken.balanceOf(charlie) +
            balanceManager.getBalance(charlie, quoteCurrency);

        uint256 charlieBaseIncrease = charlieCurrentBaseBalance -
            charlieInitialBaseBalance;
        uint256 charlieQuoteDecrease = charlieInitialQuoteBalance -
            charlieCurrentQuoteBalance;

        console.log("charlie base increase", charlieBaseIncrease);
        console.log("charlie maker fee", charlieMakerFee);
        console.log(
            "charlie base increase",
        ...
        assertApproxEqAbs(
            ...
            charlieExpectedLocked,
            100,
            "Charlie's quote token decrease incorrect"
        );

        // 6. Verify David received his base tokens
        uint256 davidCurrentBaseBalance = baseToken.balanceOf(david) +
            balanceManager.getBalance(david, baseCurrency);
        uint256 davidCurrentQuoteBalance = quoteToken.balanceOf(david) +
            balanceManager.getBalance(david, quoteCurrency);

        uint256 davidBaseIncrease = davidCurrentBaseBalance -
            davidInitialBaseBalance;
        uint256 davidQuoteDecrease = davidInitialQuoteBalance -
            davidCurrentQuoteBalance;

        assertApproxEqAbs(
            davidBaseIncrease,
            davidQuantityUnwrapped - davidMakerFee,
            100,
        ...
        assertApproxEqAbs(
            ...
            davidExpectedLocked,
            100,
            "David's quote token decrease incorrect"
        );

        // Check that orderbook is now empty at those price levels
        (uint48 orderCount1, uint256 volume1) = orderBook.getOrderQueue(
            Side.BUY,
            alicePrice
        );
        (uint48 orderCount2, uint256 volume2) = orderBook.getOrderQueue(
            Side.BUY,
            charliePrice
        );
        (uint48 orderCount3, uint256 volume3) = orderBook.getOrderQueue(
            Side.BUY,
            davidPrice
        );
        ...
        assertEq(
            orderCount1,
            0,
            "Orders should be fully matched at price 2505"
        );
        assertEq(
            orderCount2,
            0,
            "Orders should be fully matched at price 2502"
        );
        assertEq(
            orderCount3,
            0,
            "Orders should be fully matched at price 2500"
        );
        ...
        console.log(
            "  Amount: 2505 USDC at 2505 WETH/USDC =",
            aliceTradeValue,
            "USDC"
        );
        ...
        console.log(
            "  Amount: 2502 USDC at 2502 WETH/USDC =",
            charlieTradeValue,
            "USDC"
        );
        ...
        console.log(
            "  Amount: 2500 USDC at 2500 WETH/USDC =",
            davidTradeValue,
            "USDC"
        );
        ...
        public
    {
        ...
        // Define order parameters
        Price alicePrice = Price.wrap(2500 * (10 ** quoteDecimals)); // Alice's limit price

        // Alice places a large sell order (maker)
        Quantity aliceQuantity = Quantity.wrap(
            uint128(5 * (10 ** baseDecimals))
        ); // 5 WETH

        // Multiple takers will place market orders to buy from Alice
        Quantity bobQuantity = Quantity.wrap(uint128(1 * (10 ** baseDecimals))); // 1 WETH
        Quantity charlieQuantity = Quantity.wrap(
            uint128(2 * (10 ** baseDecimals))
        ); // 2 WETH
        Quantity davidQuantity = Quantity.wrap(
            uint128((3 * (10 ** baseDecimals)) / 2)
        ); // 1.5 WETH

        // Calculate expected locked amounts
        uint256 aliceExpectedLocked = Quantity.unwrap(aliceQuantity);

        // Alice places a large sell limit order (maker)
        vm.startPrank(alice);
        console.log(
            "--- Alice places sell order at 2500 USDC per WETH (5 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        uint256 totalFilledQuantity = bobQuantityUnwrapped +
            charlieQuantityUnwrapped +
            davidQuantityUnwrapped;

        // Expected remaining quantity in Alice's order
        uint256 expectedRemainingQuantity = aliceQuantityUnwrapped -
            totalFilledQuantity;
        ...
        uint256 totalTradeValue = bobTradeValue +
            charlieTradeValue +
            davidTradeValue;

        uint256 bobTradeValueInQuote = (bobTradeValue *
            (10 ** quoteDecimals) *
            alicePriceUnwrapped) /
            (10 ** quoteDecimals) /
            (10 ** baseDecimals);
        uint256 charlieTradeValueInQuote = (charlieTradeValue *
            (10 ** quoteDecimals) *
            alicePriceUnwrapped) /
            (10 ** quoteDecimals) /
            (10 ** baseDecimals);
        uint256 davidTradeValueInQuote = (davidTradeValue *
            (10 ** quoteDecimals) *
            alicePriceUnwrapped) /
            (10 ** quoteDecimals) /
            (10 ** baseDecimals);
        uint256 totalFilledQuantityInQuote = totalFilledQuantity *
            (10 ** quoteDecimals) *
            alicePriceUnwrapped /
            (10 ** quoteDecimals) /
            (10 ** baseDecimals);
        ...
        // Maker fee (for Alice)
        uint256 aliceMakerFee = ((((totalFilledQuantity *
            (10 ** quoteDecimals)) / (10 ** baseDecimals)) * feeMaker) /
            feeUnit) * (alicePriceUnwrapped / (10 ** quoteDecimals));
        ...
        uint256 bobQuoteDecrease = bobInitialQuoteBalance -
            bobCurrentQuoteBalance;

        assertApproxEqAbs(
            bobBaseIncrease,
            bobQuantityUnwrapped - bobTakerFee,
            100,
            "Bob's base token increase incorrect"
        );
        assertApproxEqAbs(
            bobQuoteDecrease,
            bobTradeValueInQuote,
            100,
            "Bob's quote token decrease incorrect"
        );
        ...
        assertApproxEqAbs(
            davidBaseIncrease,
            davidQuantityUnwrapped - davidTakerFee,
            100,
            "David's base token increase incorrect"
        );
        assertApproxEqAbs(
            davidQuoteDecrease,
            davidTradeValueInQuote,
            100,
            "David's quote token decrease incorrect"
        );
        ...
        uint256 aliceBaseDecrease = aliceInitialBaseBalance -
            aliceCurrentBaseBalance;
        uint256 aliceQuoteIncrease = aliceCurrentQuoteBalance -
            aliceInitialQuoteBalance;

        assertApproxEqAbs(
            aliceBaseDecrease,
            totalFilledQuantity,
            100,
            "Alice's base token decrease incorrect"
        );
        assertApproxEqAbs(
            aliceQuoteIncrease,
        ...
        assertApproxEqAbs(
            ...
            100,
            "Alice's remaining locked balance incorrect"
        );

        // 7. Verify the order book state after all market orders
        (uint48 orderCountAfter, uint256 volumeAfter) = orderBook.getOrderQueue(
            Side.SELL,
            alicePrice
        );
        ...
        if (expectedRemainingQuantity > 0) {
            ...
            assertApproxEqAbs(
                ...
                expectedRemainingQuantity,
                100,
                "Remaining volume at Alice's price incorrect"
            );
        } else {
            assertEq(
                orderCountAfter,
                0,
                "Alice's order should be fully matched and removed"
            );
    ...
    function testMultipleMakersSingleTakerPartialOrderMatchingWithHigherMakerQuantity() public {
        ...
        Quantity charlieQuantity = Quantity.wrap(
            uint128(3 * (10 ** baseDecimals))
        ); // 3 WETH = 7506 USDC
        Quantity davidQuantity = Quantity.wrap(
            uint128(4 * (10 ** baseDecimals))
        ); // 4 WETH = 10000 USDC

        // Define a smaller sell quantity that will only partially fill the buy orders
        Quantity bobQuantity = Quantity.wrap(
            uint128((3 * (10 ** baseDecimals)) / 2)
        ); // 1.5 WETH

        // Calculate expected locked amounts for buy orders
        uint256 aliceExpectedLocked = (Price.unwrap(alicePrice) *
            Quantity.unwrap(aliceQuantity)) / (10 ** baseDecimals);
        uint256 charlieExpectedLocked = (Price.unwrap(charliePrice) *
            Quantity.unwrap(charlieQuantity)) / (10 ** baseDecimals);
        uint256 davidExpectedLocked = (Price.unwrap(davidPrice) *
            Quantity.unwrap(davidQuantity)) / (10 ** baseDecimals);
        ...
        vm.startPrank(alice);
        console.log(
            "--- Alice places buy order at 2505 USDC per WETH (2 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        vm.startPrank(charlie);
        console.log(
            "--- Charlie places buy order at 2502 USDC per WETH (3 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        vm.startPrank(david);
        console.log(
            "--- David places buy order at 2500 USDC per WETH (4 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        uint256 expectedMatchVolumeInQuote = (bobQuantityUnwrapped *
            alicePriceUnwrapped) / (10 ** baseDecimals);
        ...
        assertApproxEqAbs(
            aliceBaseIncrease,
            bobQuantityUnwrapped - aliceMakerFee,
            100,
            "Alice didn't receive correct base tokens"
        );

        // Alice's quote tokens spent should be proportional to the filled amount
        uint256 aliceQuoteSpent = (bobQuantityUnwrapped * alicePriceUnwrapped) /
            (10 ** baseDecimals);
        uint256 aliceQuoteLockedAmount = balanceManager.getLockedBalance(
            alice,
            address(orderBook),
            quoteCurrency
        );
        ...
        assertApproxEqAbs(
            aliceQuoteDecrease,
            aliceQuoteSpent + aliceQuoteLockedAmount,
            100,
            "Alice's quote token decrease incorrect"
        );
        ...
        assertApproxEqAbs(
            ...
            100,
            "Alice should have remaining locked balance"
        );
        ...
        assertEq(
            charlieBaseIncrease,
            0,
            "Charlie's base balance should not change"
        );
        assertApproxEqAbs(
            charlieQuoteDecrease,
            charlieExpectedLocked,
        ...
        assertEq(
            davidBaseIncrease,
            0,
            "David's base balance should not change"
        );
        assertApproxEqAbs(
            davidQuoteDecrease,
            davidExpectedLocked,
            100,
            "David's quote token decrease should only be locked amount"
        );

        // Check orderbook state after partial matching
        (uint48 orderCountAfterAlice, uint256 volumeAfterAlice) = orderBook
            .getOrderQueue(Side.BUY, alicePrice);
        (uint48 orderCountAfterCharlie, uint256 volumeAfterCharlie) = orderBook
            .getOrderQueue(Side.BUY, charliePrice);
        (uint48 orderCountAfterDavid, uint256 volumeAfterDavid) = orderBook
            .getOrderQueue(Side.BUY, davidPrice);

        // Alice's order should still be in the book but with reduced volume
        assertEq(
            orderCountAfterAlice,
            1,
            "Alice's order should still be in the book"
        );
        uint256 expectedRemainingVolume = aliceQuantityUnwrapped -
            bobQuantityUnwrapped;
        assertApproxEqAbs(
            volumeAfterAlice,
        ...
        assertEq(
            orderCountAfterCharlie,
            1,
            "Charlie's order should still be in the book"
        );
        assertEq(
            orderCountAfterDavid,
            1,
            "David's order should still be in the book"
        );
        assertEq(
            volumeAfterCharlie,
            charlieQuantityUnwrapped,
            "Volume at Charlie's price should be unchanged"
        );
        assertEq(
            volumeAfterDavid,
            davidQuantityUnwrapped,
            "Volume at David's price should be unchanged"
        );
        ...
        console.log(
            "  Untouched - still in order book: 3 WETH at 2502 USDC/WETH"
        );

        console.log("David's order:");
        console.log(
            "  Untouched - still in order book: 4 WETH at 2500 USDC/WETH"
        );
    ...
    function testMultipleMakersSingleTakerPartialOrderMatchingWithHigherTakerQuantity() public {
        ...
        Quantity bobQuantity = Quantity.wrap(uint128(6 * (10 ** baseDecimals))); // 6 WETH (but only 4.5 WETH available from makers)
        ...
        vm.startPrank(alice);
        console.log(
            "--- Alice places buy order at 2505 USDC per WETH (1 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        vm.startPrank(charlie);
        console.log(
            "--- Charlie places buy order at 2502 USDC per WETH (1.5 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        vm.startPrank(david);
        console.log(
            "--- David places buy order at 2500 USDC per WETH (2 WETH) ---"
        );
        router.placeOrderWithDeposit(
            baseCurrency,
            quoteCurrency,
        ...
        uint256 totalAvailableLiquidity = aliceQuantityUnwrapped +
            charlieQuantityUnwrapped +
            davidQuantityUnwrapped;
        console.log(
        ...
        uint256 aliceTradeValue = (aliceQuantityUnwrapped *
            alicePriceUnwrapped) / (10 ** baseDecimals);
        uint256 charlieTradeValue = (charlieQuantityUnwrapped *
            charliePriceUnwrapped) / (10 ** baseDecimals);
        uint256 davidTradeValue = (davidQuantityUnwrapped *
            davidPriceUnwrapped) / (10 ** baseDecimals);
        uint256 totalTradeValue = aliceTradeValue +
            charlieTradeValue +
            davidTradeValue;
        ...
        uint256 charlieMakerFee = (charlieQuantityUnwrapped * feeMaker) /
            feeUnit;
        ...
        uint256 charlieRemainingLocked = balanceManager.getLockedBalance(
            charlie,
            address(orderBook),
            quoteCurrency
        );
        assertEq(
            charlieRemainingLocked,
            0,
            "Charlie should have no remaining locked balance"
        );
        ...
        uint256 davidRemainingLocked = balanceManager.getLockedBalance(
            david,
            address(orderBook),
            quoteCurrency
        );
        assertEq(
            davidRemainingLocked,
            0,
            "David should have no remaining locked balance"
        );

        // 6. Verify Bob has remaining quantity that wasn't matched
        uint256 bobRemainingQuantity = bobQuantityUnwrapped -
            totalAvailableLiquidity;
        ...
        (uint48 orderCountBob, uint256 volumeBob) = orderBook.getOrderQueue(
            Side.SELL,
            Price.wrap(1)
        );
        ...
        assertEq(
            orderCountAfterAlice,
            0,
            "Alice's order should be fully matched"
        );
        assertEq(
            orderCountAfterCharlie,
            0,
            "Charlie's order should be fully matched"
        );
        assertEq(
            orderCountAfterDavid,
            0,
            "David's order should be fully matched"
        );
        assertEq(volumeAfterAlice, 0, "Volume at Alice's price should be zero");
        assertEq(
            volumeAfterCharlie,
            0,
            "Volume at Charlie's price should be zero"
        );
        ...
        console.log("Alice's trade:");
        console.log(
            "  Amount: 1 WETH at 2505 USDC/WETH =",
            aliceTradeValue,
            "USDC"
        );
        console.log("  Received: ", aliceBaseIncrease, "WETH");
        ...
        console.log("Charlie's trade:");
        console.log(
            "  Amount: 1.5 WETH at 2502 USDC/WETH =",
            charlieTradeValue,
            "USDC"
        );
        console.log("  Received: ", charlieBaseIncrease, "WETH");
        ...
        console.log("David's trade:");
        console.log(
            "  Amount: 2 WETH at 2500 USDC/WETH =",
            davidTradeValue,
            "USDC"
        );
        console.log("  Received: ", davidBaseIncrease, "WETH");
    ...
    function testMarketOrderWithNoLiquidity() public {
        ...
        Quantity bobQuantity = Quantity.wrap(uint128(2 * (10 ** baseDecimals))); // 2 WETH
        uint256 bobQuantityUnwrapped = Quantity.unwrap(bobQuantity);

        // Verify the order book is empty to start with
        (uint48 orderCountAlice, uint256 volumeAlice) = orderBook.getOrderQueue(
            Side.BUY,
            Price.wrap(2505 * (10 ** quoteDecimals))
        );
        assertEq(
            orderCountAlice,
            0,
            "Order book should be empty at this price"
        );
        ...
        assertTrue(
            !success,
            "Market order should fail when there is no liquidity"
        );

        // Verify the order book state after the attempted market order
        (uint48 finalOrderCount, uint256 finalVolume) = orderBook.getOrderQueue(
            Side.BUY,
            Price.wrap(2505 * (10 ** quoteDecimals))
        );
        ...
        (uint48 sellOrderCount, uint256 sellVolume) = orderBook.getOrderQueue(
            Side.SELL,
            Price.wrap(1)
        );
        ...
        uint256 bobFinalBaseBalance = baseToken.balanceOf(bob) +
            balanceManager.getBalance(bob, baseCurrency);
        uint256 bobFinalQuoteBalance = quoteToken.balanceOf(bob) +
            balanceManager.getBalance(bob, quoteCurrency);
        ...
        console.log(
            "WETH change:",
            int256(bobFinalBaseBalance) - int256(bobInitialBaseBalance)
        );
        console.log(
            "USDC change:",
            int256(bobFinalQuoteBalance) - int256(bobInitialQuoteBalance)
        );

        // Check locked balances
        uint256 bobLockedBaseBalance = balanceManager.getLockedBalance(
            bob,
            address(orderBook),
            baseCurrency
        );
        uint256 bobLockedQuoteBalance = balanceManager.getLockedBalance(
            bob,
            address(orderBook),
            quoteCurrency
        );
        ...
        if (bobLockedBaseBalance > 0) {
            ...
            console.log(
                "Result: Bob's tokens were taken despite no matching orders"
            );
        } else if (success) {
            console.log(
                "Result: Market order was accepted but no tokens were taken"
            );
        } else {
            console.log(
    ...
    function logBalance(string memory user, address addr) internal view {
        ...
        console.log(
            "  WETH available in contract:",
            balanceManager.getBalance(addr, baseCurrency)
        );
        console.log(
            "  WETH locked in contract:",
            balanceManager.getLockedBalance(
                addr,
                address(orderBook),
                baseCurrency
        ...
        console.log(
            "  USDC available in contract:",
            balanceManager.getBalance(addr, quoteCurrency)
        );
        console.log(
